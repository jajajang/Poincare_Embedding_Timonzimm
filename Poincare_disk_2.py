...
def plot(filename):
    ...
    targ_x, targ_y=embeddings[word2idx[target]]
    dz=-complex(targ_x,targ_y)

    for n in targets:
        temp_x, temp_y=embeddings[word2idx[n]]
        z=complex(temp_x, temp_y)
...
for epoch in range(EPOCHS):
    filename='epoch_copy_'+str(epoch)+'.png'
    plot(filename)
    bar2 = hypernyms
