...
start_ = torch.Tensor(len(uniq_hypernyms), DIM).cuda()
nn.init.uniform(start_, a=-0.001, b=0.001)
EMBEDDINGS=Variable(start_, requires_grad=True)
loss_avg=0
for epoch in range(EPOCHS):
    filename='epoch_check'+str(epoch)+'.png'
    plot(filename)
    bar2 = hypernyms
    for i, (w1, w2) in enumerate(bar2):
        i_w1 = word2idx[w1]
        i_w2 = word2idx[w2]
        u=EMBEDDINGS[i_w1].unsqueeze(0)
        v=EMBEDDINGS[i_w2].unsqueeze(0)
        NEG_SAMPLES=[]
        ...
        negs = EMBEDDINGS[NEG_SAMPLES]
        loss =-torch.log(torch.exp(-1 * distance(u, v)) / torch.exp(-1 * distance(u, negs)).sum())
        loss_avg+=loss
        loss.backward()
        if i%100==0:
            print 'Average error on '+str(i)+ " : " + str(loss_avg/100)
            loss_avg=0
            print 'u grad'
            print u.grad.data
            print 'v grad'
            ...
            print 'Mammal grad'
            print EMBEDDINGS[target_index].grad.data
        if i%1000==0:
